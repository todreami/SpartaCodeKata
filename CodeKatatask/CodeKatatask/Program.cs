...
public class Program
{
    ...
    //63번 숫자 짝꿍
    public string solutionsixtythree(string X, string Y)
    {
        string answer = "";
    ...
    //64번 체육복
    public int solutionsixtyfour(int n, int[] lost, int[] reserve)
    {
        Array.Sort(lost);
        Array.Sort(reserve);
    ...
    //65번 문자열 나누기
    public int solutionsixtyfive(string s)
    {
        int answer = 0;
        int sameIdx = 0;
        int diffIdx = 0;
    ...
    //66번 대충 만든 자판
    public int[] solutionsixtysix(string[] keymap, string[] targets)
    {
    ...
    //67번 둘만의 암호
    public string solutionsixtyseven(string s, string skip, int index)
    {
        string answer = "";
        char temp;
        for (int i = 0; i < s.Length; i++)
    ...
    //69번 성격 유형 검사하기
    public string solutionsolutionsixtynine(string[] survey, int[] choices)
    {
        string answer = "";
    ...
    //70번 바탕화면 정리
    public int[] solutionnseventy(string[] wallpaper)
    {
